 ...
 */
uint32   ftm_input_get (FTMn_e ftmn, FTM_CHn_e ch)
{
	ASSERT( (ftmn == FTM0) || ( (ftmn == FTM1 || ftmn == FTM2 ) && (ch <= FTM_CH1))
#ifdef MK60F15  
               || (ftmn == FTM3)       //FX  有FTM3
...
//        return FTM_CNT_REG(FTMN[ftmn]);
	return FTM_CnV_REG(FTMN[ftmn],ch);    //保存
}
...
void     ftm_input_clean (FTMn_e ftmn)
{
	ASSERT( ftmn < FTM_MAX);     //检查传递进来的通道是否正确
        ...
        FTM_CnV_REG(FTMN[ftmn],FTM_CH1)=0;
...
//{
//  
//extern volatile uint32 cnvtime ;                //输入捕捉值
...
//-----------------------------------------------------------------------------------
void config_pin_mode(FTMn_e ftmn, FTM_CHn_e ch, FTM_Input_cfg cfg)
{
    switch(cfg)
    {
        //输入捕捉模式下：DECAPEN = 0 ， DECAPEN = 0 ，CPWMS = 0， MSnB:MSnA = 0

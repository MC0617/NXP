...
extern uint32   ftm_input_get   (FTMn_e, FTM_CHn_e);                                //输入捕捉模式下，读取捕捉事件发生时的计数器值(根据两次捕捉之间的差值可判断周期频率)
extern void     ftm_input_clean (FTMn_e );                                          //清空计数器的值

//extern void     FTM2_Input_test_handler(void);                      //可供参考的 FTM1 输入捕捉中断服务函数
...
extern void config_pin_mode(FTMn_e ftmn, FTM_CHn_e ch, FTM_Input_cfg cfg);

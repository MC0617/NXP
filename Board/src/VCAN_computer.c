 ...
 */
void vcan_sendimg(UARTn_e uartn,void *imgaddr, uint32_t imgsize)
{
#define CMD_IMG     1
	uint8_t cmdf[2] = {CMD_IMG, ~CMD_IMG};    //山外上位机 使用的命令
	uint8_t cmdr[2] = {~CMD_IMG, CMD_IMG};    //山外上位机 使用的命令

	uart_putbuff(uartn, cmdf, sizeof(cmdf));    //先发送命令

	uart_putbuff(uartn, (uint8_t *)imgaddr, imgsize); //再发送图像

	uart_putbuff(uartn, cmdr, sizeof(cmdr));    //先发送命令
}
